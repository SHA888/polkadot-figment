...
const main = async () => {
  ...
  const message = stringToU8a("this is my message in ta bottle");
  ...
  console.log(
    `The signature ${u8aToHex(signature)}, is ${
      isValid ? " " : "in"
    }valid for message "${u8aToString(message)}"`
  );
};
